...
- (void)setDataModel:(RCDSearchResultModel *)model {
    float namelLabelWidth = SCREEN_WIDTH - 20 - 48 - 9.5;
    if (model.time) {
    ...
    if (!(model.otherInformation.length > 0) || !(model.name.length > 0)) {
        ...
        CGFloat additionalLabelWidth = SCREEN_WIDTH - 20 - 48 - 9.5;
    ...
    if (!(model.portraitUri.length > 0)) {
        ...
        self.headerView.image = [UIImage imageNamed:KDefaultPortrait];
    } else {
        [self.headerView
...
- (void)loadView {
    ...
    self.timeLabel = [[UILabel alloc] initWithFrame:CGRectMake(SCREEN_WIDTH - 100 - 10, (65 - 48) / 2, 100, 17)];
