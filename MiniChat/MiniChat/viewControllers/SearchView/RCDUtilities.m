...
+ (NSString *)defaultGroupPortrait:(RCGroup *)groupInfo {
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:filePath]) {
        ...
        UIImage *portrait = [UIImage imageNamed:KDefaultPortrait];
...
+ (NSString *)hanZiToPinYinWithString:(NSString *)hanZi {
    ...
    for (int j = 0; j < hanZi.length; j++) {
        ...
        if ([self isChinese:[hanZi substringWithRange:NSMakeRange(j, 1)]]) {
            singlePinyinLetter = @"";
            singlePinyinLetter = [[NSString stringWithFormat:@"%@", [self transformToPingyinWithString:hanZi]] uppercaseString];
        } else {
...
+ (NSString *)transformToPingyinWithString:(NSString *)string {
